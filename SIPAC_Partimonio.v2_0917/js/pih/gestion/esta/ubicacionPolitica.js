$(function() {
    ...
    var data = new Array();
    var asociadosListGraph1Data;
    var asociadosListGraph1 = $("#asociadosListGraph1").val();
    var asociadosListGraph1Str = asociadosListGraph1.split(",").slice(0, 5);

    $(asociadosListGraph1Str).each(function(rowIdx, rowVal) {
        arrVal = rowVal.split(':');
        ...
        rowHtml.push('<td class="asociados_' + rowIdx + '">' + arrVal[0] + '(' + arrVal[1] + ')</td>');
        rowHtml.push('</tr>');

        $('#asociadosLabelWrap').append(rowHtml.join(''));

        data.push({
            'label': (arrVal[1] / $('#asociadosAllCnt').val() * 100).toFixed(1),
            'value': (arrVal[1] / $('#asociadosAllCnt').val() * 100)
        });
    });
 
    var vis = d3.select(".asociados")
    ...
    $('.searchBtn').on('click', function() {
        $('#SRCH_FORM').attr('')
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/ubicacionPolitica.do');
    ...
    $('.excelDownload').on('click', function() {
        ...
        $('#strCols').val(strCols);
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/ubicacionPoliticaExcelList.do');
        $('#SRCH_FORM').submit();
    });
});
...
function zonaAmbiChk(md){
	if(md == "1"){
		$("input:checkbox[id='ambiChk']").prop("checked", false);
	}else if(md == "2"){
		$("input:checkbox[id='zonaChk']").prop("checked", false);
	}
}