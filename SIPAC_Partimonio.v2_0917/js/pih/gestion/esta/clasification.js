$(function() {
    ...
    color = d3.scale.ordinal().range(["#5B9BD5", "#ED7D31", "#A5A5A5"]);
    var data = new Array();
    var ubigeoCntGraph1Data;
    var ubigeoCntGraph1 = $("#clasficacionListGraph1").val();
    var ubigeoCntGraph1Str = ubigeoCntGraph1.split(",").slice(0, 3);

    $(ubigeoCntGraph1Str).each(function(rowIdx, rowVal) {
        arrVal = rowVal.split(':');
        ...
        rowHtml.push('<td class="ubicacion' + rowIdx + '">' + arrVal[0] + '(' + arrVal[1] + ')</td>');
        rowHtml.push('</tr>');

        $('#conCulLabelWrap').append(rowHtml.join(''));

        data.push({
            'label': (arrVal[1] / $('#condCulAllCnt').val() * 100).toFixed(1),
            'value': (arrVal[1] / $('#condCulAllCnt').val() * 100)
        });
    });
 
    var vis = d3.select(".concul")
    ...
    $(document).on('change', '[name=classCd]', function() {
        Code.stdCodeList('Y', 'HRE', '005', $(this).val(), '2', '', function(result) {
            var _html = '<option value=\"\" >-- Seleccione --</option>';
            result.forEach(function(v, i) {
    ...
    if ($('[name=classCd]').val() != '' && $('[name=classCd]').val() != null) {
        Code.stdCodeList('Y', 'HRE', '005', $('[name=classCd]').val(), '2', '', function(result) {
            var _html = '<option value=\"\" >-- Seleccione --</option>';
            result.forEach(function(v, i) {
    ...
    $('.searchBtn').on('click', function() {
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/clasification.do');
    ...
    $('.excelDownload').on('click', function() {
        ...
        $('#strCols').val(strCols);
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/clasificationExcelList.do');
        $('#SRCH_FORM').submit();
    });
