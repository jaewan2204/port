$(function() {
    ...
    var data = new Array();
    var propieCntGraph1Data;
    var propieCntGraph1 = $("#propietarioListGraph1").val();
    var propieCntGraph1Str = propieCntGraph1.split(",").slice(0, 3);
    $(propieCntGraph1Str).each(function(rowIdx, rowVal) {
        arrVal = rowVal.split(':');
        ...
        rowHtml.push('<td class="propietario_' + rowIdx + '">' + arrVal[0] + '(' + arrVal[1] + ')</td>');
        rowHtml.push('</tr>');

        $('#propietarioLabelWrap').append(rowHtml.join(''));

        data.push({
            'label': (arrVal[1] / $('#propietarioAllCnt').val() * 100).toFixed(1),
            'value': (arrVal[1] / $('#propietarioAllCnt').val() * 100)
        });
    });
    ...
    $('.searchBtn').on('click', function() {
        $('#SRCH_FORM').attr('')
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/propietario.do');
    ...
    $('.excelDownload').on('click', function() {
        ...
        $('#strCols').val(strCols);
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/propietarioExcelList.do');
        $('#SRCH_FORM').submit();
    });
});
