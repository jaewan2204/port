$(function() {
	...
	var data = new Array();
	var inscripcionGraph1Data;
	var inscripcionCntGraph1 = $("#inscripcionGraph1").val();
	var inscripcionCntGraph1Str = inscripcionCntGraph1.split(",").slice(0, 5);
	
	$(inscripcionCntGraph1Str).each(function(rowIdx, rowVal) {
	    arrVal = rowVal.split(':');
	    ...
	    rowHtml.push('<td class="inscripcion_' + rowIdx + '">' + arrVal[0] + '(' + arrVal[1] + ')</td>');
	    rowHtml.push('</tr>');
	
	    $('#inscripcionLabelWrap').append(rowHtml.join(''));
	
	    data.push({
	        'label': (arrVal[1] / $('#inscripcionAllCnt').val() * 100).toFixed(1),
	        'value': (arrVal[1] / $('#inscripcionAllCnt').val() * 100)
	    });
	});
 
	var vis = d3.select(".inscripcion")
    ...
    $('.searchBtn').on('click', function() {
        $('#SRCH_FORM').attr('')
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/integrante.do');
    ...
    $('.resetBtn').on('click', function() {
        ...
        $("input:radio[name='culConRegYn']:radio[value='Y']").prop('checked', true); 
    ...
    $('.excelDownload').on('click', function() {
        ...
        $('#strCols').val(strCols);
        $('#SRCH_FORM').attr('action', '/pih/pihEstadistica/integranteExcelList.do');
        $('#SRCH_FORM').submit();
    });
});
