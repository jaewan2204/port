$(function () {
   ...
   var privadoCnt = 0;
   var publicoCnt = 0;
   var publicoCntMcCnt = 0;
   $('.totalData').each(function(i, v){
      if($(v).data('tpOfOwnCd') == '08600001'){
		privadoCnt = privadoCnt + $(v).data('totalCnt');
      }else if($(v).data('tpOfOwnCd') == '08600002'){
		publicoCnt = publicoCnt + $(v).data('totalCnt');
      }else if($(v).data('tpOfOwnCd') == '08600003'){
		publicoCntMcCnt = publicoCntMcCnt + $(v).data('totalCnt');
      }
   });
   arr.push({
       totalCnt: privadoCnt,
   ...
   arr.push({
       totalCnt: publicoCntMcCnt,
       labelText: 'Público MC'
   });
   
	arr.push(privadoCnt);
	arr.push(publicoCnt);
	arr.push(publicoCntMcCnt);
   ...
   $(arr).each(function(rowIdx, rowObj) {
    ...
    var vis = d3.select(".tipoPropietario")
   ...
   var juridicaCnt = 0;
   $('.totalData').each(function(i, v){
      if($(v).data('tpOwnCd') == '08500001'){
		naturalCnt = naturalCnt + $(v).data('totalCnt');
      }else if($(v).data('tpOwnCd') == '08500002'){
		juridicaCnt = juridicaCnt + $(v).data('totalCnt');
      }
   });
   arr.push({
       totalCnt: naturalCnt,
       labelText: 'Persona natural'
   });
   arr.push({
       totalCnt: juridicaCnt,
       labelText: 'Persona juridica'
   });
   ...
   $(arr).each(function(rowIdx, rowObj) {
	...
	//Propietario 1Depth selectBox Change
	$('#tpOwnCd').on('change', function(){
		var url = "/pm/pmEstadistica/getPropietarioList.ajax";
        var jsonData = {
            'tpOfOwnCd' : $('#tpOfOwnCd').val(),
	        'tpOwnCd' : $('#tpOwnCd').val()
        };
        Ajax.request(url, jsonData, function(data){
            if(data.MSG = 'success'){
            	...
            	$(data.list).each(function(i, v){
            	    _html += '<option value=\"'+v.ownNo+'\">'+v.nombres+'</option>';
	            });
            	$('#ownNo').html(_html);
            }
        }, '');
	...
	//detailOwnNo   
	if($('#detailOwnNo').val() != '' || $('#detailOwnNo').val() != null){
		var url = "/pm/pmEstadistica/getPropietarioList.ajax";
        var jsonData = {
            'tpOfOwnCd' : $('#tpOfOwnCd').val(),
	        'tpOwnCd' : $('#tpOwnCd').val()
        ...
        Ajax.request(url, jsonData, function(data){
            if(data.MSG = 'success'){
            	var _html = '<option value=\"\" >-- Seleccione --</option>';
            	$(data.list).each(function(i, v){
            		if($('#detailOwnNo').val() == v.ownNo){
            			_html += '<option value=\"'+v.ownNo+'\" selected>'+v.nombres+'</option>';
		            }else{
            			_html += '<option value=\"'+v.ownNo+'\">'+v.nombres+'</option>';
		            }
    ...
    $('.searchBtn').on('click', function(){
    	$('#SRCH_FORM').attr('')
        $('#SRCH_FORM').attr('action', '/pm/pmEstadistica/propietario.do');
    ...
    $('.resetBtn').on('click', function(){
        $('.reset').val('');
        $('#ownNo').html('<option value=\"\" >-- Seleccione --</option>');
    ...
    $('.excelDownload').on('click', function(){
        var strCols = 'Propietario,Apellidos y Nombres,Tipo de propietario,TOTAL';
        $('#strCols').val(strCols);
        $('#SRCH_FORM').attr('action', '/pm/pmEstadistica/propietarioExcelList.do');
        $('#SRCH_FORM').submit();
    });
});
