...
$(function () {
	...
	$('.modifyBtn').on('click', function(){
		...
		$.ajax({
            ...
            success: function (data) {
                if (data.result > 0) {
                	location.href='/pm/pmRegInsFicha/nuevaFicha.do?mvbGenSeq='+genSeq+'&srchMode=avanzada';;
                }
            }
        });
    });
	...
	$('.resetBtn').on('click', function(){
		location.href='/pm/pmSrchAdvance/avanzada.do';
	});
	...
	$('.searchBtn').on('click', function(){
		$('#pageIndex').val('1');
        $('#SRCH_FORM').attr('action', '/pm/pmSrchAdvance/avanzada.do?'+$("meta[name='_csrf_parameter']").attr("content")+'='+$('meta[name="_csrf"]').attr('content'));
        $("#SRCH_FORM").attr("enctype", 'multipart/form-data');
    ...
    $('#excelDownLoad').on('click', function () {
		...
		*/
        $('#SRCH_FORM').attr('method', 'post');
        $('#SRCH_FORM').attr('action', '/pm/pmSrchAdvance/avanzadaExcelList.do?'+$("meta[name='_csrf_parameter']").attr("content")+'='+$('meta[name="_csrf"]').attr('content'));
        $('#SRCH_FORM').submit();
    });
